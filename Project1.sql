...
BEGIN
    ...
    CREATE TABLE IF NOT EXISTS Descriptive (
        ...
        StdDev DECIMAL(10, 2),
        Count INT,
        lat DECIMAL(10, 6), -- Add latitude column
        lng DECIMAL(10, 6), -- Add longitude column
        PRIMARY KEY (city)
    );
    ...
    SELECT 
        ...
        MAX(cities.latitude) AS lat, -- Use an aggregate function for latitude
        MAX(cities.longitude) AS lng -- Use an aggregate function for longitude
    FROM loans_preview
    LEFT JOIN cost_of_living ON loans_preview.City = cost_of_living.City
    LEFT JOIN cities ON loans_preview.City = cities.city
    LEFT JOIN weather ON loans_preview.City = weather.City AND loans_preview.Date = weather.Date
...
# Descriptive is our new data mart
